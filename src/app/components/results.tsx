...
export function Results({ result, publicView = false }: { result: AuditResult; publicView?: boolean }) {
  ...
  const heroClass = result.verdict === "optimized" ? "bg-[var(--color-hero-optimized)]" : "bg-[var(--color-hero-savings)]";
  const inputClass = "min-h-11 w-full rounded-md border border-[var(--color-border-input)] bg-[var(--color-surface)] px-3 py-2 font-[inherit] text-[var(--color-text-primary)] transition-colors focus:border-[var(--color-accent)] focus:outline-none focus:ring-1 focus:ring-[var(--color-accent)]";
  const buttonClass = "inline-flex min-h-[46px] cursor-pointer items-center justify-center rounded-md border-0 bg-[var(--color-brand)] px-[18px] font-extrabold text-white no-underline transition-all hover:bg-[var(--color-brand-hover)] disabled:cursor-wait disabled:opacity-60";
  ...
  return (
    <section className="px-[clamp(18px,4vw,56px)] pb-16">
      <div className={`mt-[38px] grid items-center gap-6 p-[clamp(26px,5vw,54px)] text-white min-[981px]:grid-cols-[minmax(0,1fr)_260px] ${heroClass}`}>
        <div>
          <p className="mb-3.5 text-xs font-extrabold uppercase tracking-[0.12em] text-[var(--color-accent)]">AI spend audit</p>
          ...
          <p className="text-[1.35rem] font-extrabold text-[var(--color-text-annualized)]">${result.totalAnnualSavings.toLocaleString()} annualized</p>
        </div>
      ...
      <p className="max-w-[980px] text-xl leading-[1.65] text-[var(--color-text-body)]">{result.summary}</p>
      ...
      <div className="mt-[18px] grid gap-2.5">
        {result.recommendations.map((item) => (
          <article className="grid items-center gap-4 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-[18px] min-[981px]:grid-cols-[220px_minmax(0,1fr)_120px]" key={`${item.toolId}-${item.currentPlan}`}>
            <div>
              <p className="m-0 mb-1 font-extrabold">{item.toolName}</p>
              <span className="leading-snug text-[var(--color-text-muted)]">{item.currentPlan}: ${item.currentSpend.toLocaleString()}/mo</span>
            </div>
            <div>
              <p className="m-0 mb-1 font-extrabold">{item.recommendedAction}</p>
              <span className="leading-snug text-[var(--color-text-muted)]">{item.reason}</span>
            </div>
            <strong className="text-left text-lg text-[var(--color-savings)] min-[981px]:text-right">${item.monthlySavings.toLocaleString()}/mo</strong>
          </article>
        ))}
      </div>
      ...
      {!publicView && (
        <div className="mt-6 grid gap-4 min-[981px]:grid-cols-2">
          <form className="grid gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-[22px]" onSubmit={captureLead}>
          ...
          <div className="rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-[22px]">
            ...
            <p className="text-[var(--color-text-secondary)] [overflow-wrap:anywhere]">{shareUrl || "Saving public URL..."}</p>
